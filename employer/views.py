...
class ConfirmPayment555(APIView):
    permission_classes = [IsAuthenticated]
        ...
        try:
            ...
            if response_data['status'] == 'success':
                ...
                if (
                    ...
                    transaction.status = response_data['data']["processor_response"]
                    transaction.transaction_id = response_data['data']['id']
                   

                    flutter_transaction_id = response_data['data']['id']
                    flutter_transaction_ref_id = response_data['data']['tx_ref']
                    flutter_app_fee = response_data['data']['app_fee']
                    flutter_settled_amount = response_data['data']['amount_settled']
                    flutter_card_issuer  = response_data['card']['issuer']
                    card_type = response_data['card']['type']
                    ip_address = response_data['data']['ip']
                    first_6digits = response_data['card']['first_6digits']
                    last_4digits  = response_data['card']['last_4digits']
                    modified_at  = response_data['card']['created_at']
                    device_fingerprint = response_data['data']['device_fingerprint']

                    transaction.save()

                    logger.info(f"Transaction status updated to 'Payment Completed'.{response_data['data']['status']}")
...
class ConfirmPaymentnnn(APIView):
    permission_classes = [IsAuthenticated]
                ...
                try:
                    ...
                    if (
                        ...
                        with transaction.atomic():
                            ...
                            # Log before saving
                            logger.info(f"Transaction details before saving: {transaction}")
                            transaction.save()
